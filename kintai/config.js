...
/* ── みなし休憩 ──────────────────────────────────────────
   休憩開始・休憩終了の打刻がない日に、休憩を取ったものとして差し引く時間。
   休憩ボタンはふだん使わないため、これがないと休憩0分になり、
   拘束時間がそのまま労働時間として集計されてしまう。

   社員ごとに変えたい場合は、社員マスタの BreakMin に分数を入れる。
   0 を入れればその社員には差し引かない。
   ------------------------------------------------------------ */
var KT_BREAK = {
  defaultMin: 60,   // 既定の休憩（分）。社員マスタの BreakMin があればそちらが優先
  minSpanMin: 360   // 拘束時間がこれ以下の日は差し引かない（6時間＝休憩の義務がない）
};
...
var KT_PUNCH = {
  ...
  confirmOutMin: 60,     // 出勤からこの分数以内に退勤を押したら一度確認する
  closingTime:   '17:00',// 定時。これより前に退勤を押したら一度確認する（'' で確認しない）
  lockAfterOut:  true,   // 退勤したらその日は出勤を押せなくする（同じ日に出直す運用なら false）
  undoMin:       15      // 打刻直後この分数は、その場に［取消］を出す
};

/* ── 給与の締めと支払い ──────────────────────────────────
   当社は「26日締め・翌月払い」。
   たとえば 7/27〜8/26 の勤務が、9月に支払われます。
   月次集計と CSV はこの区切りで出します（暦月ではありません）。
   ------------------------------------------------------------ */
var KT_PAY = {
  closingDay:  26,  // 締め日。前月の締め日の翌日から、当月のこの日まで
  payMonthLag: 1    // 支払いは締め月の何か月あとか（1 = 翌月）
};
...
/* 年休として消化する種別（代休はここに含めない） */
var KT_LEAVE_PAID_TYPES = ['全日', '午前半休', '午後半休', '時間単位'];

/* ── 代休 ────────────────────────────────────────────────
   休日に出勤したあと、別の日を休みにする扱い。

   ★ 代休を取っても、休日出勤の割増賃金の支払い義務は消えません。
     相殺できるのは賃金の100%部分だけで、法定休日の35%（所定休日で
     週40時間を超えた分は25%）の上乗せ分は別途支払う必要があります。

   事前に休日と労働日を入れ替える「振替休日」は割増が不要ですが、
   それは代休ではなく KintaiHolidays リストで日の種別を入れ替えて表します。
   ------------------------------------------------------------ */
var KT_COMP = {
  fullDayMin:   480,   // 休日にこれ以上働いたら代休 1.0日（8時間）
  halfDayMin:   240,   // これ以上なら 0.5日（4時間）
  expireMonths: 3,     // 発生から何か月以内に取得するか（就業規則で定めること）
  alertDays:    30     // 期限までこの日数を切ったら警告する
};
