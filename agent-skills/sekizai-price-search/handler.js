...
// AnythingLLM (Docker) でのストレージパス:
// このファイル: /app/server/storage/plugins/agent-skills/sekizai-price-search/handler.js
// カスタム文書: /app/server/storage/custom-documents/
...
module.exports = {
  name: 'sekizai-price-search',
  setup(aibitat) {
    aibitat.function({
      super: aibitat,
      name: this.name,
      description: '商品価格表を検索します。商品名・カテゴリ・仕入先で検索でき、販売価格・仕入価格・編集URLを返します。価格・仕入先に関する質問には必ずこのツールを使ってください。',
      parameters: {
        ...
        properties: {
          query: {
            type: 'string',
            description: '検索する商品名・カテゴリ名・仕入先名'
          }
        },
        required: ['query']
      },
