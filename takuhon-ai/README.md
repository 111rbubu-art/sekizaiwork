...
```
① 墨出し      拓本      → 綺麗な墨       （正解＝人が直した拓本の墨。ふだんの学習）
...
実際に、②の学習（フォントの癖・1850 組）が①「墨出し（拓本）」の欄に
...
**いちばん上の［① 墨出し（拓本）｜② 整える（書体）］が、ページぜんぶを切り替える。**
...
**ページぜんぶが**、**①「墨出し」と②「整える」で別々**。
いちばん上の **［① 墨出し（拓本）｜② 整える（書体）］** で切り替える。
...
- **列（枠の材料）** … 登録した切り抜き（`dataset/lines/*/raw.png`）の上に、
  **人が直した枠**を赤でそのまま重ねて出す。読みの入っている枠は、字も出る。
  ...
  - **［◀ 前の列］［次の列 ▶］… 列から列へ続けて入れる**（2026-09-22。本人の指示
    「文字を入れる画面で、次の列へや前の列に移動できるようにして」）。
    移るときは、その列を**自動で保存**する（打ったものが消えない）。
    `Alt` ＋ `←` `→` でも移れる。
    並びは**画面を開いたときのもの**を控えて使う（保存すると `order=new` の
    並びが変わるので、そのつど拾い直すと［次の列］が行ったり来たりする）。
    ［保存する］も［前／次］も、読みと枠を **`/boxes` で一度に**書き込む
...
### 読みの学習を回すには（2026-09-22。本人の問い「実行はどうすれば良いのですか」）

［③ 文字枠・読み（拓本）］の **［④ 読み（どの字か当てる）］→［読みの学習を始める］**。
材料は ① 列の枠に入れた読み（`dataset/lines`）と ② 手本帳（`dataset/chars`）の合わせ技で、
**1 字につき 3 枚以上ある字が 2 種以上**ないと `state:"few"` で終わる。

そこで、ボタンのすぐ下に**回せるかどうか**を出すようにした（`bcReady()`）。
- 回せるとき … 「回せます：学習に入る字 ○ 種（○ 枚）」
- 足りないとき … 「まだ回せません：3 枚以上ある字が ○ 種しかありません」＋
  **あと何枚で 3 枚になる字**の一覧（`令2・和2` の 2 は「あと 2 枚」）
- ［試すだけ］に レ点を入れると `--min-per-char 1` で回る（通るかどうかの確かめ用）
...
- `stage` は `ink`（既定・① 墨出し）／`shape`（② 整える）／`both`（①のあと②）。
